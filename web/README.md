...
> Примітка: у продакшені демо-підказки в інтерфейсі вимкнені; ці обліковки наведені лише для локальної розробки після `db:seed`.
...
Реєстрація: **власник чи орендар** (`tenancyType`). Голосування та теми форуму мають поле **аудиторії** (усі / лише власники / лише орендарі); голова бачить усе. Окремої ролі «адмін платформи» поки немає — панель `/chair` для голови та модератора (модератор: користувачі + конфіденційні звернення).
